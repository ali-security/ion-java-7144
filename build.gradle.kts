...
tasks {
     ...
     * to run all tests for each JRE, so they are not included in the `build` task when running locally.
     * However, they are included in the `build` task for a CI environment, and they are mandatory as a
     * prerequisite for publishing any release.
     */
    val minifyTest = register<Task>("minifyTest") { group = "verification" }

    publish { dependsOn(minifyTest) }
    if (isCI) build { dependsOn(minifyTest) }

    SUPPORTED_JRE_VERSIONS.map {
        register<Test>("minifyTest$it") {
            javaLauncher.set(project.javaToolchains.launcherFor { languageVersion.set(JavaLanguageVersion.of(it)) })
            applyCommonTestConfig()
            classpath = project.configurations.testRuntimeClasspath.get() + project.sourceSets.test.get().output + minifyJar.outputs.files
            dependsOn(minifyJar)
        }
    }.forEach { minifyTest { dependsOn(it) } }
